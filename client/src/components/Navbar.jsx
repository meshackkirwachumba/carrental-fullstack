  ...
  return (
    ...
    text-gray-600 border-b border-borderColor relative transition-all
     ${location.pathname === "/" && "bg-light"}`}>
        ...
        <Link to="/">
              <motion.img whileHover={{scale:1.05}} src="/favicon.svg" alt='logo' className='h-8' />
        </Link>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16
         ...
         ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
              ...
              <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
                <button
                  onClick={()=> isOwner ? navigateTo("/owner") : changeRole()}
        ...
        {/* button for opening and closing the menu in mobile screen */}
